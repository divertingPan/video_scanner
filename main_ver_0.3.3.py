...
class Application(tk.Frame):
    ...
    def start_and_status(self):
        ...
        bt_process = tk.Button(frame_bt_process, text='process', command=self.process, font=self.font)
        bt_process.pack()

        frame_bt_process = tk.Frame(self.frame_right_3)
        frame_bt_process.pack(side=tk.LEFT, padx=5, expand=tk.YES)
        bt_calculate_mov = tk.Button(frame_bt_process, text='calculate move', command=self.calculate_split_wp, font=self.font)
        bt_calculate_mov.pack()

        frame_bt_stop = tk.Frame(self.frame_right_3)
        frame_bt_stop.pack(side=tk.RIGHT, padx=5, expand=tk.YES)
        bt_stop = tk.Button(frame_bt_stop, text='stop', command=self.stop_process, font=self.font)
        bt_stop.pack()
